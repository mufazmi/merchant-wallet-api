...
class AdminWalletTransactionController {
    ...
    findAll = async (req: Request, res: Response, next: NextFunction) => {
        ...
        return data.length > 1 ? responseSuccess({ res: res, message: Messages.WALLET.WALLET_TRANSACTION_FOUND, data: data }) : next(ErrorHandler.notFound(Messages.WALLET.WALLET_TRANSACTION_NOT_FOUND));
    }
}
